...
function buildRows() {
	return ROWS.map((row, index) => {
		const seats = Array.from({ length: 10 }, (_, seatIndex) => {
			...
			let state: "available" | "gap" | "occupied" | "companion" =
				"available";
			if (seed === 0) state = "gap";
		...
		const tier =
			index < 6 ? "Plateia A" : index < 8 ? "Plateia B" : "Balcão";
...
export default function SeatMapTheaterScreen({
	event,
	onBack,
	onBuy,
}: SeatMapTheaterScreenProps) {
	...
	const tierPrice = (tier: string) =>
		event.ticketTypes.find((ticketType) => ticketType.name === tier)
			?.price ?? 0;

	function toggleSeat(
		seat: {
			id: string;
			state: "available" | "gap" | "occupied" | "companion";
		},
		tier: string,
	) {
			...
			<div className="flex-1 space-y-4 overflow-y-auto px-4 pb-4">
				...
				<div className="border-t border-neutral-800 pt-3">
					...
					<p className="mb-3 text-xs text-neutral-500">
						Toque em um assento disponível para selecioná-lo
					</p>
					...
					<div className="mb-4 space-y-2">
						{event.ticketTypes.map((ticketType) => (
							<div
								key={ticketType.name}
								className="flex items-center justify-between rounded-xl border border-neutral-800 px-4 py-2.5"
							>
