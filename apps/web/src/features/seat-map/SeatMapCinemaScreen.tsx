...
const LEGEND = [
	...
	{ label: "Cadeirante", dot: "bg-blue-400" },
];
...
export default function SeatMapCinemaScreen({ event, onBack, onBuy }: Props) {
	...
	const maxGridX = useMemo(
		() => Math.max(1, ...rows.flatMap((r) => r.seats.map((s) => s.gridX))),
		[rows],
	);
				...
				<div className="mx-auto w-full max-w-3xl space-y-4 pt-4">
					...
					<div className="border-t border-neutral-800 pt-3">
						...
						<p className="mb-3 text-xs text-neutral-500">
							{activeType?.name} · {money(activeType?.price ?? 0)}{" "}
							por assento
						</p>

						<div className="overflow-x-auto rounded-xl bg-neutral-900 p-3">
							<div className="flex min-w-[420px] flex-col items-center space-y-1.5">
								{rows.map(({ rowLabel, seats }) => (
									<div
										key={rowLabel}
										className="flex items-center gap-1.5"
									>
										<span className="w-4 text-right text-[10px] text-neutral-500">
											{rowLabel}
										</span>
										<div
											className="grid gap-1"
											style={{
												gridTemplateColumns: `repeat(${maxGridX}, minmax(0,1fr))`,
											}}
										>
											{Array.from(
												{ length: maxGridX },
								...
								<div className="mt-2 border-t border-neutral-700 pt-2 text-center text-[10px] tracking-widest text-neutral-500">
									TELA
								</div>
							</div>
						</div>
