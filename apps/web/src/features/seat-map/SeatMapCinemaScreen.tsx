...
function buildRows() {
	const rows = [] as Array<{
		row: number;
		seats: Array<{ id: string; state: "available" | "gap" | "occupied" }>;
	}>;
...
export default function SeatMapCinemaScreen({
	event,
	onBack,
	onBuy,
}: SeatMapCinemaScreenProps) {
			...
			<div className="flex-1 space-y-4 overflow-y-auto px-4 pb-4">
				...
				<div className="border-t border-neutral-800 pt-3">
					...
					<p className="mb-3 text-xs text-neutral-500">
						Inteira · {money(pricePerSeat)} por assento
					</p>
					...
					onClick={() =>
						onBuy({
							...
							ticketType: {
								name: "Inteira",
								price: pricePerSeat,
								available: event.ticketTypes[0].available,
							},
							qty: selected.size,
							total,
						})
